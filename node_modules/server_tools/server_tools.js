...
server_tools.parsePost=function(request)
{
	...
	console.log("POST IS GETTING PARSED");
	console.log(request);
	...
	if(level=="notice")
	{
		...
		if(_event=="state_change")
		{
			...
			GLOBAL.db.collection('stationlist').update({id:groundstation_id}, {$set: {state:newState}}, function()
			{
				...
				//first, let everyone know this GS is RUNNING
				//var msg={update_type:'status', data:"RUNNING", id:groundstation_id, timestamp:timestamp};
				//GLOBAL.socket.emit('update_station_data', msg);
				//next update state
		...
		if(_event[6]=="=")
		{
			...
			GLOBAL.db.collection('stationlist').update({id:groundstation_id}, {$set: {status:newStatus}}, false, false).on("end", function()
			{
				/*
		...
		if(_event=="number_of_swaps")
		{
			...
			GLOBAL.db.collection('stationlist').update({id:groundstation_id}, {$set:{number_of_swaps:number_of_swaps}}).on("end", function()
			{
				...
				console.log("MESSAGE: ");
				console.log(msg);
				socket.emit('update_station_data', msg);
			});
		}
	...
	else if(level=="alert")
	{
		...
		if(_event=="crash")
		{
			...
			GLOBAL.db.collection('stationlist').find({id:groundstation_id}).toArray(function(err, item)
			{
				...
				{
					GLOBAL.socket.emit('update_station_data', item[0]);
				});
				...
				{
					GLOBAL.socket.emit('add_error', newError);
				});
			});
