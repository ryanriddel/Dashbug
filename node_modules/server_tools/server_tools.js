...
server_tools.parsePost=function(request)
{
	//*********************************
	...
	if(level=="notice")
	{
		...
		if(_event=="state_change")
		{
			...
			db.collection('stationlist').update({id:groundstation_id}, {$set: {state:newState}}, function()
			{
				db.collection('stationlist').find({id:ground_station_id}).toArray(function(err, item)
				{
					socket.emit('update_station_data', item[0]);
				});
			});
		...
		if(_event[6]=="=")
		{
			...
			var statusBlocks=_event.split("=");
			var newStatus=statusBlocks[1];
			
			db.collection('stationlist').update({id:groundstation_id}, {$set: {status:newStatus}}, function()
			{
				db.collection('stationlist').find({id:ground_station_id}).toArray(function(err, item)
				{
					socket.emit('update_station_data', item[0]);
	...
	else if(level=="alert")
	{
		...
		{
			var newStatus="DOWN";
			db.collection('stationlist').find({id:groundstation_id}).toArray(function(err, item)
			{
				var currentState=item[0].state;
				item[0].state=responsible_module + " " + _event + " @" + currentState;
				db.collection('stationlist').update({id:groundstation_id}, {$set: {status:newStatus, state:item[0].state}}, function()
				{
						socket.emit('update_station_data', item[0]);
				});
			
				//add this to the errorlist
				db.collection('errorlist').insert({timestamp:timestamp, module:responsible_module, ground_station_id:groundstation_id, state:newState, notes:"Failed after " + currentState, error_message: _event, swaps_before_error:number_of_swaps}, function()
				{
	...
	else if(level=="error")
	{
		...
		if(_event=="timeout")
		{
			...
			db.collection('stationlist').find({id:groundstation_id}).toArray(function(err, item)
			{
				...
				db.collection('stationlist').update({id:groundstation_id}, {$set: {status:newStatus, state:newState}});

				//add this to the errorlist
				db.collection('errorlist').insert({timestamp:timestamp, module:responsible_module, ground_station_id:groundstation_id, state:newState, notes:"Failed after " + currentState, error_message: _event, swaps_before_error:number_of_swaps});
			});
